...
| 3 | Alternate reading schedules | Support reading plans beyond the Bible Companion (e.g. other Christadelphian or general plans) the user can choose from. | — | L/XL | ✅ Done (Alt Sprints A–E) | Shipped: **two** gate-verified, live-switchable, per-plan-progress plans — **Bible Companion** (flagship default, byte-for-byte unchanged for non-switchers) + the classic **4-stream M'Cheyne** — selectable in Settings, with a non-destructive switch (own `plan_id` progress partition; zero-loss v1→v2 Room migration) and N-stream-correct day cards / stats / strips / widget / reminders. Identity evolved to "a reading-plan app whose flagship is the Bible Companion." Owner OQs resolved: OQ-3 per-plan progress, OQ-4 date-anchored/365-day only, OQ-MC 4-stream M'Cheyne, OQ-7 Settings-only selector. **Chronological plan = NO-GO** (recorded in [data/README.md](data/README.md)): no *named* chronological ordering has two genuinely-independent agreeing sources (the contested-ordering / re-mirror dilemma — D-ALT-21 honesty gate), so it was correctly NOT shipped; the feature is complete + valuable with two plans. Plan/eng/exec: [features/alternate-reading-schedules.md](features/alternate-reading-schedules.md), [ENGINEERING_SPEC-alternate-schedules.md](ENGINEERING_SPEC-alternate-schedules.md), [EXECUTION_PLAN-alternate-schedules.md](EXECUTION_PLAN-alternate-schedules.md). Handoffs: sprint-alt-A…E. Release blocked on the owner device pass + string sign-offs, then the 1.5.0/10500 bump + rollout. |

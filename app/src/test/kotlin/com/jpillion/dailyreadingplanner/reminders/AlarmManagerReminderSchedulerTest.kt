...
class AlarmManagerReminderSchedulerTest {
    ...
    private val scheduler =
        AlarmManagerReminderScheduler(context, FakeDateProvider(LocalDate(2026, 6, 10), now = now))
    ...
    private fun epochMilliAtUtc(
        ...
        LocalDateTime(2026, 6, dayOfMonth, hour, minute)
            .toInstant(TimeZone.UTC)
            .toEpochMilliseconds()

    @Test
    fun `scheduleReminder arms one inexact RTC_WAKEUP alarm at the next occurrence`() {
        scheduler.scheduleReminder(LocalTime(8, 0))
    ...
    fun `rescheduling replaces the standing alarm instead of stacking a second one`() {
        scheduler.scheduleReminder(LocalTime(8, 0))
        scheduler.scheduleReminder(LocalTime(21, 30))

...
class ProgressRepositoryPlanScopeTest {
    ...
    fun setUp() {
        database =
            Room
                ...
                .build()
        repository = ProgressRepositoryImpl(database.readingProgressDao(), dateProvider)
    }
    ...
    fun `counts strips and clearYear are all scoped per plan`() =
        runTest {
            ...
            assertThat(repository.readCounts(start, end, planId = mcheyne).first()).containsOnly(d1 to 1)
        }
    ...
    fun `hasAnyMarks is GLOBAL - true for a mark under any plan`() =
        runTest {
            ...
            repository.setRead(LocalDate(2026, 6, 1), 3, isRead = true, planId = mcheyne)
    ...
    fun `the default planId argument reads the flagship plan - parity`() =
        runTest {
            ...
            val other = LocalDate(2026, 3, 16)
            repository.setRead(other, 1, isRead = true)
            assertThat(repository.streamsRead(other, planId = bc).first()).containsExactlyInAnyOrder(1)
        }
}
...
class ProgressMigrationNoPerceptibleChangeTest {
    ...
    fun `migrated v1 history reads back identically through the new default-plan repository`() =
        runTest {
            ...
            val day1 = LocalDate(2026, 1, 1) // whole day
            val day2 = LocalDate(2026, 6, 10) // partial (NT only)
            ...
            helper.createDatabase(dbName, 1).use { v1 ->
                listOf(1, 2, 3).forEach { s ->
                    v1.execSQL(
                        ...
                        arrayOf<Any>(day1.toEpochDays(), s, 1_700_000_000_000 + s),
                    )
                }
                v1.execSQL(
                    "INSERT INTO reading_progress (dateEpochDay, stream, readAtEpochMillis) VALUES (?, ?, ?)",
                    arrayOf<Any>(day2.toEpochDays(), 3, 1_700_000_006_010),
                )
            }
            ...
            val db =
                Room
                    ...
                    .build()
            val repo = ProgressRepositoryImpl(db.readingProgressDao(), dateProvider)

            // 4. The default-plan read returns EXACTLY the pre-migration history.
            assertThat(repo.streamsRead(day1).first()).containsExactlyInAnyOrder(1, 2, 3)
            assertThat(repo.streamsRead(day2).first()).containsExactlyInAnyOrder(3)
            assertThat(repo.readCounts(LocalDate(2026, 1, 1), LocalDate(2026, 12, 31)).first())
                .containsOnly(day1 to 3, day2 to 1)
            assertThat(repo.allReadCounts().first()).containsOnly(day1 to 3, day2 to 1)

...
class ReadingFormatterTest {
    ...
    fun `non-consecutive chapters of the same book do not collapse`() {
        val p = portion(2, "Psalms" to 5, "Psalms" to 7)
    ...
    fun `book boundary breaks a run even when chapter numbers are consecutive`() {
        val p = portion(1, "Genesis" to 50, "Exodus" to 1)
    ...
    fun `the singular Psalm only applies to a single chapter not a range - mutation guard`() {
        ...
        assertThat(ReadingFormatter.format(portion(2, "Psalms" to 23)))
            .isEqualTo("Psalm 23")
        assertThat(ReadingFormatter.format(portion(2, "Psalms" to 1, "Psalms" to 2)))
            .isEqualTo("Psalms 1–2")
    }

    @Test
    fun `the singular rule is Psalms-specific - other single-chapter books are unchanged`() {
        assertThat(ReadingFormatter.format(portion(3, "Obadiah" to 1)))
            .isEqualTo("Obadiah 1")
        assertThat(ReadingFormatter.format(portion(3, "Philemon" to 1)))
    ...
    fun `the abbreviated form keeps Psa for both single and multiple chapters - D-UI-2`() {
        ...
        assertThat(ReadingFormatter.formatAbbreviated(portion(2, "Psalms" to 23)))
            .isEqualTo("Psa 23")
        assertThat(
            ReadingFormatter.formatAbbreviated(windowedPortion(2, "Psalms", 119, 1, 40)),
        ).isEqualTo("Psa 119:1–40")
        assertThat(
            ReadingFormatter.formatAbbreviated(portion(2, "Psalms" to 1, "Psalms" to 2)),
        ).isEqualTo("Psa 1–2")
    }
    ...
    fun `abbreviated form uses derived display abbrevs with the same range collapse - D-S9-1`() {
        ...
        val psalms = portion(2, "Psalms" to 1, "Psalms" to 2)
        assertThat(ReadingFormatter.formatAbbreviated(psalms)).isEqualTo("Psa 1–2")
        val matthew = portion(3, "Matthew" to 1, "Matthew" to 2)
    ...
    fun `all four Psalm 119 days render their windows`() {
        assertThat(ReadingFormatter.format(windowedPortion(2, "Psalms", 119, 41, 80)))
            .isEqualTo("Psalm 119:41–80")
        assertThat(ReadingFormatter.format(windowedPortion(2, "Psalms", 119, 81, 128)))
            .isEqualTo("Psalm 119:81–128")
        assertThat(ReadingFormatter.format(windowedPortion(2, "Psalms", 119, 129, 176)))
            .isEqualTo("Psalm 119:129–176")
    }

    @Test
    fun `a single-verse window renders without a range`() {
        val p = windowedPortion(2, "Psalms", 119, 7, 7)
